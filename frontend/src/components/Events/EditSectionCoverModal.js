// frontend/src/components/Events/EditSectionCoverModal.js
...
const EditSectionCoverModal = ({ open, onClose, sectionId, currentImages = [], onSave }) => {
  ...
  const handleRemoveImage = async (imageUrl) => {
    ...
    try {
      ...
      await sectionsAPI.updateCoverImages(sectionId, newImages);
      onSave(newImages);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const response = await sectionsAPI.uploadCoverImage(
          sectionId,
          formData,
          (progress) => {
      ...
      await sectionsAPI.updateCoverImages(sectionId, allImages);
...
export default EditSectionCoverModal;